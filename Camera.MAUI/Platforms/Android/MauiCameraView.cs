...
internal class MauiCameraView: GridLayout
{
    ...
    public event MauiCameraViewCallBack.ImageAvailableEventHandler ImageAvailable;

    public void RegisterCallbackForProcessGrabbedImage(MauiCameraViewCallBack.ImageAvailableEventHandler callback)
    {
        ImageAvailable += callback;
    }

    public void UnregisterCallbackForProcessGrabbedImage(MauiCameraViewCallBack.ImageAvailableEventHandler callback)
    {
        ImageAvailable -= callback;
    }
...
#pragma warning restore CS0618 // El tipo o el miembro están obsoletos
    ...
    {
        var debugTimeGap = (DateTime.Now - cameraView.lastSnapshot).TotalSeconds;
        if (!snapping && cameraView != null && cameraView.AutoSnapShotSeconds > 0 && debugTimeGap >= cameraView.AutoSnapShotSeconds)
        {
